...
def getHtml(site):
    ...
    url = "https://nhentai.net/g/{0}/1/"
    urlhtml = url.format(site)

    req = urllib.request.Request(url=urlhtml, headers=headers)
    data = urllib.request.urlopen(req).read()
    soup = BeautifulSoup(data, "html.parser")

    section = soup.find_all('section', class_='pagination')
    tag = section[0]

    tag2 = tag.find_all('a')[3]
    page_url = tag2.get('href')
    max_page = page_url.split("/")[3]

    img_class = soup.find_all("img", class_="fit-horizontal")
    imgurl = img_class[0].get("src")
    imgpath = imgurl.split("/")[4]

    return max_page, imgpath
...
def getUrlHtml(site):
    ...
    section = soup.find_all('div', class_='gallery')
    sitelist = []
    for tag in section:
        ahref = tag.find_all('a')[0]
        href = ahref.get('href')
        href = href.split("/")[2]
        sitelist.append(href)
    return sitelist
...
class CrawlerScheduler(object):
    ...
    def _download_media(self, site, medium_type, start):
        ...
        maxPage, imgpath = getHtml(site)
        base_url = "https://i.nhentai.net/galleries/" + imgpath + "/{0}.jpg"
        num = int(maxPage)
        print(base_url)
        for i in range(num):
            photo_path = base_url.format(i + 1)
            self.queue.put((medium_type, photo_path, target_folder))
        # while True:
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        ...
        urlFileName = os.path.join(cur_dir, "sites_url.txt")
        ...
        if os.path.exists(urlFileName):
            sitesUrl = parse_sites(urlFileName)
        else:
            usage()
            sys.exit(1)
    else:
        sites = sys.argv[1].split(",")
        sitesUrl = sys.argv[1].split(",")

    for site in sitesUrl:
        print("site=" + site)
        sites2 = getUrlHtml(site)
        sites = sites+sites2
